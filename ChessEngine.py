...
class GameState():
    def __init__(self):
        ...
        #self.inCheck: True nếu Vua đang bị Chiếu.
        #self.checkMate: True nếu game kết thúc bằng Chiếu bí (một bên không có nước đi hợp lệ và Vua đang bị chiếu).
        #self.staleMate: True nếu game kết thúc bằng Hòa (một bên không có nước đi hợp lệ nhưng Vua không bị chiếu).
        self.inCheck = False
        self.checkMate = False
        self.staleMate = False
        self.enpassantPossible = ()
        self.enpassantPossibleLog = [self.enpassantPossible]
        #self.pins: Một list để lưu trữ thông tin về các quân cờ đang bị ghim (pin). Một quân cờ bị ghim là quân cờ không thể di chuyển vì nếu nó di chuyển, Vua của mình sẽ bị Chiếu.
        self.pins = []
        #self.checks: Một list để lưu trữ thông tin về các quân cờ đang chiếu (check) Vua.
        self.checks = []
    ...
    def getValidMoves(self):
        ...
        if self.inCheck:
            #Vua bị chiếu Đơn (chỉ bởi MỘT quân địch)
            if len(self.checks) == 1:
                ...
                #Tạo một danh sách rỗng tên là validSqs (các ô hợp lệ). Danh sách này sẽ chứa những ô mà quân của mình có thể di chuyển tới để chặn đường chiếu hoặc bắt quân đang chiếu.
                validSqs = []
                if pieceChecking[1] == 'N':
                    validSqs = [(checkRow, checkCol)]
                else:
                    for i in range(1, 8):
                        #khi vua bị chiếu đơn thì ngoài quân mã, các nước đi hợp lệ là các nước đi nằm trên đường 
                ...
                for i in range(len(moves) - 1, -1, -1):
                    if moves[i].pieceMoved[1] != 'K':
                        #loại bỏ các nước của quân khác nếu nó không phải validmove(trừ vua) khi vua đang bị chiếu 
                        if not (moves[i].endRow, moves[i].endCol) in validSqs:
                            moves.remove(moves[i])
            else:
                #bị 2 quân chiếu cùng lúc thì phải đi vua 
                self.getKingMoves(kingRow, kingCol, moves)
        else:
            #nếu không bị chiếu, lấy tất cả các nước đi có thể 
            moves = self.getAllPossibleMoves()
    ...
    def getPawnMoves(self, r, c, moves):
        ...
        if self.whiteToMove:
            if self.board[r-1][c] == "--":
             ...
             # Tốt trắng ăn chéo sang trái
            if c-1 >= 0: # Đảm bảo không vượt quá biên trái bàn cờ
    ...
    def checkForPinsAndChecks(self):
        pins = []    # Danh sách để lưu trữ các quân cờ bị ghim
        checks = []  # Danh sách để lưu trữ thông tin về các quân cờ đang chiếu vua
        inCheck = False # Biến boolean, True nếu vua bị chiếu
        if self.whiteToMove:
            enemyColor = 'b'
            allyColor = 'w'
            ...
            for i in range(1, 8):
                ...
                if 0 <= endRow < 8 and 0 <= endCol < 8:
                    ...
                    if endPiece[0] == allyColor and endPiece[1] != 'K':
                        if possiblePins == ():
                            #Quân này có thể bị ghim. Thông tin của nó (vị trí và hướng) được lưu vào possiblePins.
                        ...
                        if (0 <= j <= 3 and pieceType == 'R') or \
                            ...
                            #Nếu quân địch tìm thấy không phải là quân đang chiếu Vua theo hướng này (ví dụ: một Tượng đứng trên đường ngang, hoặc một Mã). Quân địch này vẫn chặn đường nhìn của Vua. Không thể có chiếu hoặc ghim nào xa hơn sau quân này
                            break
                else:
                    #không làm gì cả, chỉ chuyển sang bước i tiếp theo
                    break
...
class castleRight():
    #wks, wqs, ... la quyen nhap thanh white ben knight hoac queen dang true false
